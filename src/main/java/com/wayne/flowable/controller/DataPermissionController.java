...
public class DataPermissionController {
    ...
    public String deploy() {
        ...
        Deployment deployment = processEngine.getRepositoryService().createDeployment()
                .name("数据申请流程")
                .addClasspathResource("flowable/data_permission.bpmn20.xml")
                .deploy();
    ...
    public ApiResult<String> add(String employeeId, String managerId, int days, String desc) {
        ...
        String[] v = { "jack", "tom", "admin"};
        map.put("assigneeList", Arrays.asList(v));
        map.put("opManagerId", managerId);
        map.put("applyLeaderId","zhangsan");
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("data_request_process", map);
    ...
    public ApiResult<String> listTask() {
        ...
        list.addAll( taskService.createTaskQuery().taskCandidateGroup("opManagerId").list());
        list.addAll(taskService.createTaskQuery().taskCandidateOrAssigned("zhangsan").list());
    ...
    public ApiResult<Object> show(String taskId){
        Map<String, Object> processVariables = taskService.getVariables(taskId);
        return ApiResult.success(processVariables);
    }
    /**
     * 获取process列表
     */
